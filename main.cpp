...
int main()
{
  ...
  while (true)
  {
    ...
    if (fct.type == Executable)
    {
      ...
      // std::cout<<"command_with_full_path "<<command_with_full_path<<std::endl;

      std::string exact_path="";
      int pos=command_with_full_path.size();
      while(pos--){
        if(command_with_full_path[pos] != '/' )
        {exact_path=command_with_full_path[pos]+exact_path;}
        else {break;}
      }
      command_with_full_path=exact_path;
      // std::cout<<"command_with_full_path "<<command_with_full_path<<std::endl;
      for (int argn = 1; argn < command_vector.size(); argn++)
      {
        command_with_full_path += " ";
        command_with_full_path += command_vector[argn];
        // std::cout<<"command_vector[argn] "<<command_vector[argn]<<std::endl;
      }

      const char *command_ptr = command_with_full_path.c_str();
      // std::cout<<"command ptr "<<*command_ptr<<std::endl;
